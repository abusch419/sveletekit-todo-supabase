...
<form class="my-6">
  <div class="flex flex-col text-sm mb-2">
    { #each $events as show }
    <label style="text-transform: capitalize;" class="font-bold mb-2 text-gray-800"
      for="event-checkbox">{eventLabel(show)}
      <input class="float-left mt-1 mr-1" id='event-checkbox' type="checkbox" bind:group={shows}
...
<ul>
  {#if $userEvents.length > 0}
    ...
    {/each}
  {:else}
    <p class="text-lg text-center text-gray-800 mt-4">Get started by selecting all of the shows you've attended 💃</p>
  {/if}
</ul>
